...
class Expectimax(AbstractEngine):
    def __init__(self, playing_as: Literal[0, 1]):
        ...
        self.planned_moves = []
        self.inverter_uncertainty = (
            False  # True if algo has used inverter and hasn't shot yet
        )
    ...
    def choice(self, board: BuckshotRoulette):
        ...
        if out_move == None:
            ...
            for item in board.items[self.me]:
                if item in own_moves and item not in [
                    "adrenaline",
                    "cigarettes",
                    "burner_phone",
                    "meds",
                ...
                for item in board.items[1 - self.me]:
                    if item in own_moves and item not in [
                        ...
                        em_ad_moves.extend(
                            [item]
                            * min(
                                board.items[1 - self.me][item],
                                board.items[self.me].adrenaline,
                            )
                        )
    ...
    def expectimax(
        ...
        def search(
            ...
            if (
                depth == 0
                or len(prob_map) == 0
                or pos >= len(self.known_shells)
                or (len(own_moves) + len(op_moves)) == 0
            ):
                return 0, []
            ...
            for move in own_moves:
                ...
                if move == "op":
                    ...
                    combined_val = (prob_map[0] * val_live) + (
                        (1 - prob_map[0]) * val_blank
                    )
        ...
        prob_map = [
            X / N if shell == None else float(shell) for shell in self.known_shells
        ]
        ...
        val, moves = search(
            ...
            0,
            False,
            depth=len(own_moves) + len(op_moves),
        )
        return moves
...
class OptimalAlgo(AbstractEngine):
        ...
        def criticality(op_items: Items):
            ...
            if X == 0:
                if op_items.inverter > 0:
                    max_hits = (
                        1
                        if op_items.handcuffs == 0
                        else min(2, min(op_items.inverter, N))
                    )
                else:
                    return 0
            else:
                max_hits = (
                    1
                    if op_items.handcuffs == 0
                    else min(2, X + min(op_items.inverter, N))
                )
    ...
    def choice(self, board: BuckshotRoulette):
        ...
        if not self.inverter_uncertainty:
            ...
            if live == 0:
                ...
                self.known_shells = [
                    False if shell == None else shell for shell in self.known_shells
                ]
                ...
                unknown_round_chance = None
                self.known_shells = [
                    True if shell == None else shell for shell in self.known_shells
                ]
            else:
        ...
        if out_move == None:
            if "meds" in moves:
                ...
                if (
                    "cigarettes" in moves
                    and board.max_charges - board.charges[self.me] > 1
                ):
                    # Cancel out risk, take the 50/50 for 2 lives
                    out_move = "cigarettes"
    ...
    def move_tree(
        ...
        unk_N = (
            N
            - self.known_shells[depth:].count(True)
            - self.known_shells[depth:].count(False)
        )
        ...
        for move in moves:
            ...
            match move:
                ...
                case "saw":
                    ...
                    move_vals[move] = (
                        2
                        * self.move_tree(next_moves, X, N, next_items, depth, known)[1]
                    )
                case "beer":
                    next_items.beer -= 1

                    live_case = self.move_tree(
                        next_moves, X - 1, N - 1, next_items, depth + 1, known
                    )[1]
                    blank_case = self.move_tree(
                        next_moves, X, N - 1, next_items, depth + 1, known
                    )[1]

                    move_vals[move] = (hit_prob * live_case) + (
                        (1 - hit_prob) * blank_case
                    )
                case "inverter":
                    ...
                    live_case = self.move_tree(
                        next_moves, X - 1, N, next_items, depth, known
                    )[1]
                    blank_case = self.move_tree(
                        next_moves, X + 1, N, next_items, depth, known
                    )[1]
                ...
                case "magnifying_glass":
                    ...
                    live_case = self.move_tree(
                        next_moves, X, N, next_items, depth, known=True
                    )[1]
                    blank_case = self.move_tree(
                        next_moves, X, N, next_items, depth, known=False
                    )[1]
    ...
    def find_best_move(self, moves: list[str], board: BuckshotRoulette) -> Literal[
        "op",
        "self",
        "handcuffs",
        "magnifying_glass",
        ...
        for move in moves:
            match move:
                case "op":
                    move_vals[move] = (X / N, (X**2 * (N - X)) / N**3)
                case "saw":
                    move_vals[move] = (2 * X / N, 2 * (X**2 * (N - X)) / N**3)
                case "self":
                    move_vals[move] = (
                        X * (1 - X) / (N * (N - 1)),
                        -(X**2)
                ...
                case "inverter":
                    move_vals[move] = ((N - X) / N, X * (N - X) ** 2 / N**3)
                case "magnifying_glass":
                    move_vals[move] = (
                        X * (2 * N - X - 1) / (N * (N - 1)),
                        X
