...
class BuckshotRouletteEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Dict({
            ...
            'items_player_self': spaces.Box(low=0, high=8, shape=(len(self.game.POSSIBLE_ITEMS),), dtype=np.int32),
            'items_player_op': spaces.Box(low=0, high=8, shape=(len(self.game.POSSIBLE_ITEMS),), dtype=np.int32),
            'items_active': spaces.Box(low=0, high=8, shape=(len(self.game.POSSIBLE_ITEMS),), dtype=np.int32),
            'current_turn': spaces.Discrete(2),
            'shell_knowledge': spaces.Box(low=-1, high=1, shape=(8,), dtype=np.int32),
    ...
    def step(self, action):
        ...
        known_shells = self.shell_knowledge_player_0 if self.game.current_turn == 0 else self.shell_knowledge_player_1
        ...
        self._update_shell_knowledge(chosen_move, result)
        
        known_shells = self.shell_knowledge_player_0 if self.game.current_turn == 0 else self.shell_knowledge_player_1
        known_heur = (1 - (known_shells.count(None) / len(self.game._shotgun))) - known_heur
        
        # Determine if the game is over and who won
        done = self.game.winner() is not None
        
        reward = 0.03 * known_heur
        reward += 0.02 * (8 - len(self.game._shotgun))
        if chosen_move == 'op':
            # Either won the game, or did 0-2 damage to opponent
            reward = max(reward, result * 0.24)
        if chosen_move == 'self':
            # Either lost the game, or did 0-2 damage to self
            reward = min(reward, result * 0.25)
            
        reward = 1 if self.game.winner() == current_player else -1 if self.game.winner() == 1 - current_player else reward
    ...
    def _update_shell_knowledge(self, last_move, move_result):
        """ Updates the shell knowledge of the current player based on the move and its result """
        current_player = self.game.current_turn
        shell_knowledge = self.shell_knowledge_player_0 if current_player == 0 else self.shell_knowledge_player_1
        ...
        match last_move:
            ...
            case 'magnifying_glass':
                shell_knowledge[0] = 1 if move_result else 0  # Update first shell with true (1) or false (0)
                self.inverter_uncertainty_0 = False
                self.inverter_uncertainty_1 = False
            case 'burner_phone':
                if move_result != None:
                    shell_knowledge[move_result[0]] = 1 if move_result[1] else 0  # Update specific shell from burner phone result
            case 'inverter':
                if self.shell_knowledge_player_0[0] is None:
                    self.inverter_uncertainty_0 = True
                else:
                    self.shell_knowledge_player_0[0] = not self.shell_knowledge_player_0[0]
                
                if self.shell_knowledge_player_1[0] is None:
                    self.inverter_uncertainty_1 = True
                else:
                    self.shell_knowledge_player_1[0] = not self.shell_knowledge_player_1[0]
            
        if not self.inverter_uncertainty_0:
            self.live_0 = self.game.shotgun_info()[0]
            if self.game._shotgun.count(False) == self.shell_knowledge_player_0.count(False):
                self.shell_knowledge_player_0 = [x if x is not None else True for x in self.shell_knowledge_player_0]
            if self.game._shotgun.count(True) == self.shell_knowledge_player_0.count(True):
                self.shell_knowledge_player_0 = [x if x is not None else False for x in self.shell_knowledge_player_0]
        if not self.inverter_uncertainty_1:
            self.live_1 = self.game.shotgun_info()[0]
            if self.game._shotgun.count(False) == self.shell_knowledge_player_1.count(False):
                self.shell_knowledge_player_1 = [x if x is not None else True for x in self.shell_knowledge_player_1]
            if self.game._shotgun.count(True) == self.shell_knowledge_player_1.count(True):
                self.shell_knowledge_player_1 = [x if x is not None else False for x in self.shell_knowledge_player_1]
            
    
    def _get_observation(self):
        """ Helper function to gather the current game state """
        shell_knowledge = self.shell_knowledge_player_0 if self.game.current_turn == 0 else self.shell_knowledge_player_1
        return {
            'charges_self': self.game.charges[self.game.current_turn],
            'charges_op': self.game.charges[1 - self.game.current_turn],
            'items_player_self': np.array([self.game.items[self.game.current_turn][item] for item in self.game.POSSIBLE_ITEMS], dtype=np.int32),
            'items_player_op': np.array([self.game.items[1 - self.game.current_turn][item] for item in self.game.POSSIBLE_ITEMS], dtype=np.int32),
            'current_turn': self.game.current_turn,
            'shell_knowledge': np.array([1 if x is True else -1 if x is False else 0 for x in shell_knowledge], dtype=np.int32),
            'shell_count':len(self.game._shotgun),
            'live_shell_count': self.live_0 if self.game.current_turn == 0 else self.live_1
        }
