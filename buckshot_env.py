...
class BuckshotRouletteEnv(gym.Env):
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "items_player_self": spaces.Box(
                    low=0,
                    high=8,
                    shape=(len(self.game.POSSIBLE_ITEMS),),
                    dtype=np.int32,
                ),
                "items_player_op": spaces.Box(
                    low=0,
                    high=8,
                    shape=(len(self.game.POSSIBLE_ITEMS),),
                    dtype=np.int32,
                ),
                "items_active": spaces.Box(
                    low=0,
                    high=8,
                    shape=(len(self.game.POSSIBLE_ITEMS),),
                    dtype=np.int32,
                ...
                "shell_knowledge": spaces.Box(
                    low=-1, high=1, shape=(8,), dtype=np.int32
                ),
    ...
    def step(self, action):
        ...
        reward = (
            ...
            else -1 if self.game.winner() == 1 - current_player else reward
        )
